...
def srt(path, out):
    ...
    for fr in t["freezes"]:
        ...
        events.append((fr["start_s"], end, label))
    for ev in t["stutter"]["events"]:
        events.append((ev["t_s"], ev["t_s"] + INSTANT_S, f"stutter {r4(ev['delta_ms'])}ms"))
    for cut in scenes(path)["cuts"]:
        events.append((cut, cut + INSTANT_S, "scene cut"))
    try:
    ...
    for sil in silences:
        events.append((sil["start_s"], sil["start_s"] + sil["duration_s"],
                       f"silence {r4(sil['duration_s'])}s"))
    events.sort(key=lambda e: (e[0], e[1], e[2]))
    events = events[:EVENT_CAP]
    lines = []
    for i, (start, end, label) in enumerate(events, 1):
